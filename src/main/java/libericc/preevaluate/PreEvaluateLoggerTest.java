...
public class PreEvaluateLoggerTest {
	
	public static void methodInvoke(String method, String args, int hash) {
		System.out.println(String.format("libericc: M: %s#%d=(%s)", method, hash, args));
	}
	
	public static void branchResult(String method, int branchId, int value, int hash){
		System.out.println(String.format("libericc: B: %s:%d#%d=%d", method, branchId, hash, value));
	}
