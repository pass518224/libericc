...
import libericc.edgefunc.bundle.BundleGetFloatEdge;
...
public class BundleGetFloatHandler extends CallHandler {
	@Override
	public Set<MethodSig> getTargets(){
		Set<MethodSig> targets = new HashSet<MethodSig>();
		targets.add(new MethodSig("android.os.Bundle", "float getFloat(java.lang.String)"));
		targets.add(new MethodSig("android.os.Bundle", "float getFloat(java.lang.String,float)"));
		return targets;
	}
		...
		if (callSite instanceof DefinitionStmt) {
			...
			if (callNode.equivTo(base) && returnSideNode.equivTo(lvalue) && arg0 instanceof StringConstant) {
				if (argc == 2) {
					...
					if (arg1 instanceof FloatConstant) {
						return new BundleGetFloatEdge(((StringConstant) arg0).value, ((FloatConstant) arg1).value);
					}
				}
				return new BundleGetFloatEdge(((StringConstant) arg0).value);
			}
		}
