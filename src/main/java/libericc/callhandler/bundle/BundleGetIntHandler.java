...
import libericc.edgefunc.bundle.BundleGetIntEdge;
...
public class BundleGetIntHandler extends CallHandler {
	@Override
	public Set<MethodSig> getTargets(){
		Set<MethodSig> targets = new HashSet<MethodSig>();
		targets.add(new MethodSig("android.os.Bundle", "int getInt(java.lang.String)"));
		targets.add(new MethodSig("android.os.Bundle", "int getInt(java.lang.String,int)"));
		return targets;
	}
			...
			if (callNode.equivTo(base) && returnSideNode.equivTo(lvalue) && arg0 instanceof StringConstant) {
				if (argc == 2) {
					...
					if (arg1 instanceof IntConstant) {
						return new BundleGetIntEdge(((StringConstant) arg0).value, ((IntConstant) arg1).value);
					}
				}
				return new BundleGetIntEdge(((StringConstant) arg0).value);
			}
		}
