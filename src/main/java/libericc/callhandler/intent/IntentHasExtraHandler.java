...
import libericc.edgefunc.intent.IntentHasExtraEdge;
...
public class IntentHasExtraHandler extends CallHandler {
	@Override
	public Set<MethodSig> getTargets(){
		Set<MethodSig> targets = new HashSet<MethodSig>();
		targets.add(new MethodSig("android.content.Intent", "boolean hasExtra(java.lang.String)"));
		return targets;
	}
		...
		if (callSite instanceof DefinitionStmt) {
			...
			if (arg0 instanceof StringConstant) {
				if (callNode.equivTo(base) && returnSideNode.equivTo(lvalue)) {
					return new IntentHasExtraEdge(((StringConstant) arg0).value);
				}
			}
		}
