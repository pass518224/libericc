...
import libericc.value.DataValue;
...
public class SeperatedEvaluator implements Evaluator {
	Analyzer analyzer;
	Decider decider;
	...
	public SeperatedEvaluator(Analyzer analyzer) {
	...
	public void evaluate(Decider decider) {
		...
		/*Logger.getLogger().finest("Method to hash to Args: "+ method2hash2arg.toString());
		Logger.getLogger().finest("Reachable Method to Args: "+ reachableMethod2args.toString());*/
				...
				for (int branchId=0; branchId < branchList.size(); branchId++) {
					...
					if (stmt instanceof IfStmt) {
						...
						case Success:
							reachedBranch++;
							branchSuccess++;
							if (decider.branchPredictResult((IfStmt)stmt).toTriLogic()!=TriLogic.Unknown) {
								branchSuccessWithoutAllPath++;
								Logger.getLogger().finer(String.format("Analysis Success without all path at Branch %s:%d %s", sm, branchId, stmt));
							}
							Logger.getLogger().finer(String.format("Analysis Success at Branch %s:%d %s", sm, branchId, stmt));
							break;
						case Failed:
							reachedBranch++;
							branchFailed++;
							Logger.getLogger().finer(String.format("Analysis Failed at Branch %s:%d %s", sm, branchId, stmt));
							break;
						case UnableToDetermine:
							reachedBranch++;
							branchUnableToDetermine++;
							Logger.getLogger().finer(String.format("Unable to Determine at Branch %s:%d %s", sm, branchId, stmt));
							break;
						case NotReached:
							unreachedBranch++;
						}
						...
						case Success:
							...
							branchSuccess++;
							it.remove();
							Logger.getLogger().finer(String.format("Analysis Success at Switch %s:%d %s", sm, branchId, stmt));
							break;
						case Failed:
							branchUnableToDetermine--;
							branchFailed++;
							it.remove();
							Logger.getLogger().finer(String.format("Analysis Failed at Switch %s:%d %s", sm, branchId, stmt));
							break;
						case UnableToDetermine:
							Logger.getLogger().finer(String.format("Unable to Determine at Switch %s:%d %s", sm, branchId, stmt));
							break;
						case NotReached:
							unreachedBranch++;
						}
		...
		Logger.getLogger().info("Call Site Count: "+callSiteCount);
		Logger.getLogger().info("Reached Call Site Count: "+reachedCallSite);
		Logger.getLogger().info("Call Arg Count: "+callArgCount);
		Logger.getLogger().info("Call Arg Const Count: "+callArgConstCount);
		Logger.getLogger().info("Resolved Call Arg Count: "+resolvedArgCount);
		Logger.getLogger().info("Unresolved Call Arg Count: "+unresolvedArgCount);
		Logger.getLogger().info("Reached Branches: "+reachedBranch);
		Logger.getLogger().info("Unreached Branches: "+unreachedBranch);
		Logger.getLogger().info("Branch Success: "+branchSuccess);
		Logger.getLogger().info("Branch Success without all path: "+branchSuccessWithoutAllPath);
		Logger.getLogger().info("Branch Failed: "+branchFailed);
		Logger.getLogger().info("Branch Unable to determine: "+branchUnableToDetermine);
	}
	...
	void setReachableMethod2args(){
		...
		Logger.getLogger().finest(queue.toString());
		while (!queue.isEmpty()) {
			...
			if (args.contains(ma.args)) continue;
				...
				args.add(ma.args);
				Logger.getLogger().finest("Add "+ma.toString());
				try {
					for (Unit u: ma.sm.retrieveActiveBody().getUnits()) {
						...
						if (((Stmt)u).containsInvokeExpr()) {
							...
							Map<Value, GeneralValue> map = analyzer.resultsAt(callSite);
							
							// for each callee
							Logger.getLogger().finest("\t"+u);
							for (SootMethod callee: icfg.getCalleesOfCallAt(u)) {
								Logger.getLogger().finest("\t\t"+callee);
								// match method args
	...
	boolean matchArgs(GeneralValue[] gvs, String[] argStrings) {
		Logger.getLogger().finest("gvs: "+Arrays.toString(gvs));
		Logger.getLogger().finest("arg: "+Arrays.toString(argStrings));
	...
	void loadAdbLog() {
		try (BufferedReader br = new BufferedReader(new FileReader(Config.adblogPath))) {
			Pattern patternMethod = Pattern.compile("libericc: M: (?<method>.+)#(?<hash>-?\\d+)=\\((?<argument>.*)\\)");
			Pattern patternBranch = Pattern.compile("libericc: B: (?<method>.+):(?<branchId>\\d+)#(?<hash>-?\\d+)=(?<result>-?\\d+)");
	...
	EvaluateResult evaluateBranch(String method, IfStmt stmt, int branchId) {
		TriLogic analysisResult = decider.branchPredictResult(stmt).toTriLogic();
	...
	EvaluateResult evaluateSwitch(String method, SwitchStmt stmt, int branchId) {
		...
		ResultOfSwitch analysisResult = decider.switchResult(stmt);
