...
import libericc.callhandler.Handlers;
import libericc.edgefunc.*;
...
public class ConstantPropagationProblem extends DefaultJimpleIDETabulationProblem<Value, GeneralValue, InterproceduralCFG<Unit,SootMethod>> {
	...
	ConstantPropagationUtility utility = new ConstantPropagationUtility(this);
	
	public ConstantPropagationProblem(InterproceduralCFG<Unit,SootMethod> icfg) {
		this(icfg, Scene.v().getMainMethod());
	}
	
	public ConstantPropagationProblem(InterproceduralCFG<Unit,SootMethod> icfg, SootMethod mainMethod) {
		super(icfg);
	...
	protected JoinLattice<GeneralValue> createJoinLattice(){
		return new libericc.lattice.GeneralLattice();
	}	
	...
	protected EdgeFunctions<Unit, Value, SootMethod, GeneralValue> createEdgeFunctionsFactory(){
		return new EdgeFunctions<Unit, Value, SootMethod, GeneralValue>() {
			...
			public EdgeFunction<GeneralValue> getNormalEdgeFunction(Unit curr, Value currNode, Unit succ, Value succNode){
				...
				Logger.getLogger().finest(String.format("%s: %s -> %s [%s]", curr, currNode, succNode, ret));
				return ret;
			}
			...
			public EdgeFunction<GeneralValue> getCallEdgeFunction(Unit callStmt, Value srcNode, SootMethod destinationMethod, Value destNode) {
				...
				Logger.getLogger().finest(String.format("(Call) %s, %s: %s -> %s [%s]", callStmt, destinationMethod, srcNode, destNode, ret));
				return ret;
			}
			...
			public EdgeFunction<GeneralValue> getReturnEdgeFunction(Unit callSite, SootMethod calleeMethod, Unit exitStmt, Value exitNode, Unit returnSite, Value retNode){
				...
				Logger.getLogger().finest(String.format("(Ret) %s, %s, %s: %s -> %s [%s]", callSite, calleeMethod, exitStmt, exitNode, retNode, ret));
				return ret;
			}
			...
			public EdgeFunction<GeneralValue> getCallToReturnEdgeFunction(Unit callSite, Value callNode, Unit returnSite, Value returnSideNode){
				...
				Logger.getLogger().finest(String.format("(C2R) %s: %s -> %s [%s]", callSite, callNode, returnSideNode, ret));
				return ret;
			}
		};
	}
