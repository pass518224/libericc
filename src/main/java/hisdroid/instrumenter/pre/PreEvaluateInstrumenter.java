...
public class PreEvaluateInstrumenter {
	...
	static String evaloggerPackage = "hisdroid.instrumenter.pre";
	...
	Local instrumentMethod(SootMethod method) {
		...
		Local inst_i = Jimple.v().newLocal("instrument_i", IntType.v());
		method.retrieveActiveBody().getLocals().add(inst_i);
		Local inst_l = Jimple.v().newLocal("instrument_l", IntType.v());
		method.retrieveActiveBody().getLocals().add(inst_l);
		Local tmpBundle = Jimple.v().newLocal("instrument_tmpBundle", RefType.v("android.os.Bundle"));
		method.retrieveActiveBody().getLocals().add(tmpBundle);
		...
		unitChain.insertBeforeNoRedirect(u, firstStmt);
		u = Jimple.v().newInvokeStmt(Jimple.v().newStaticInvokeExpr(Scene.v().getMethod("<hisdroid.instrumenter.pre.PreEvaluateLogger2: void methodInvoke(java.lang.String,java.lang.String,int)>").makeRef(), StringConstant.v(method.toString()), tmpstring, hash));
		unitChain.insertBeforeNoRedirect(u, firstStmt);
